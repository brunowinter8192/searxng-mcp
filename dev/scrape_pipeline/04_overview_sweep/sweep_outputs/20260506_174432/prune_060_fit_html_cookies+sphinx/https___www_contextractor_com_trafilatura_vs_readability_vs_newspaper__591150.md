...
**readability-lxml** outputs HTML. That's it. One format. You get a `summary()` method that returns cleaned HTML wrapped in a `<body>` tag, and a `title()` method. No date, no author (well, version 0.8.2 added basic author extraction), no categories. It's deliberately minimal.
