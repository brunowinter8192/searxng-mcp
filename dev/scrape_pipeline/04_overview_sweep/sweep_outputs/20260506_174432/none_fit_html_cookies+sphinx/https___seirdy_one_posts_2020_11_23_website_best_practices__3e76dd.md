...
# Best practices for inclusive textual websites
  * Posted 2020-11-23 by Seirdy on their Website and Gemini capsule. 
  * Last updated 2024-12-07. Changelog
...
## Table of Contents
  1. Intro­duction
    1. Inclusive design
    2. Restricted enhancement
    3. Prior art
    4. Why this article exists
  2. Security and privacy
    1. TLS
    2. Scripts and the Content Security Policy
    3. If you must enable scripts
    4. Third-party content
  3. Optimal loading
    1. Blocking resources
    2. Don’t count on the cache
    3. Inline content
    4. Transfer size and user flows
    5. Core Web Vitals aren’t enough
    6. Round trips
    7. The golden kilobyte
    8. Layout shifts
    9. Other server-side tweaks
  4. Against lazy loading
    1. Against speculative pre-loading
    2. Can’t users on poor connections disable images?
    3. Related issues
  5. Long-page performance
    1. CSS containment
    2. Performance of assistive technologies
  6. Old browsers
    1. Old browsers in use
    2. How to support old browsers
  7. The Tor Browser
    1. Constraints of the Tor Browser
    2. Hidden services
  8. In-page search
    1. The importance of proofreading
    2. Problematic overrides
  9. About fonts
    1. But most users don’t change their fonts…
    2. Style­sheet overrides aren’t an excuse
    3. Font fingerprinting concerns
    4. Zoom and font size
  10. Beyond alt-text
    1. Putting images in context
    2. Figures
    3. Image transcripts
  11. About custom colors
    1. Default colors
    2. Color overrides and accessibility
    3. Dark themes
    4. Use the Advanced Perceptual Contrast Algorithm
    5. What contrast algorithms don’t cover: over-saturation
    6. Contrast under different conditions
  12. Visible interactive semantics
    1. In defense of link underlines
    2. Buttons versus links
    3. Visible non-interactive semantics
    4. Against focusable containers
  13. Image optimization
    1. Breaking conventional wisdom
    2. Enforce image compression
    3. Dark image variants
    4. SVG images
  14. Layout
    ...
    4. Sidebar pitfalls
    5. Sidebar alternatives
    6. Line length
  15. Small viewports
    1. Narrow viewports are popular
    2. Wide items
    3. Keeping text together
    4. Pictures of text
    5. Indented elements
    6. Short viewports
  16. Spacing
    1. Non-interactive space
    2. Tap targets
    3. Line spacing
  17. Non-browsers: reading mode
  18. Machine translation
    1. POSH helps translation engines
    2. Changing text direction
    3. Current limitations
  19. In­accessible default stylesheets
    1. Mono­space handling
    2. Focus indicators
  20. Screen reader improve­ments
    1. Split elements and possessive hyperlinks
    2. Other tips for screen readers
  21. Future users
    1. Third parties
    2. Dead links
    3. Reproducibility
  ...
  23. Future updates
  24. Conclusion
  25. Ack­nowledge­ments and further reading
...
### Inclusive design
My primary focus is inclusive design. Specifically, I focus on supporting _underrepresented ways to read a page_. Not all users load a page in a common web-browser and navigate effortlessly with their eyes and hands. Authors often neglect people who read through accessibility tools, tiny viewports, machine translators, “reading mode” implementations, the Tor network, printouts, hostile networks, and uncommon browsers, to name a few. I list more niches in the conclusion. Compatibility with so many niches sounds far more daunting than it really is: if you only selectively override browser defaults and use plain-old, semantic HTML (POSH), you’ve done half of the work already.
...
—HTML Living Standard, section 4.2.5.3: Pragma directives, Content Security Policy state
...
**Code snippet 2**: my website includes a `link` header to load the image that serves as my IndieWeb photo and favicon. The header includes a priority hint so the browser starts downloading the resource right away. 
```
...
Nowadays, servers employ BBR-based congestion control. It allows for regular “spikes” in window size, but the initial window size is still small. Find more details in the slides from TCP and BBR (application/pdf) by Geoff Huston from APNiC.
...
—<img>: The Image Embed element on MDN,  the `loading` attribute
...
An alternative to the `loading` attribute that I _do_ recommend is the `decoding` attribute. I typically use `decoding="async"` so that image decoding can be deferred.
### CSS containment
...
To ship custom fonts is to assert that branding is more important than user choice. That might very well be a reasonable thing to do; branding isn’t evil! That being said, textual websites in particular don’t benefit much from branding. Beyond basic layout and optionally supporting dark mode, authors generally shouldn’t dictate the presentation of their websites; that should be the job of the user agent. Most websites are not important enough to look completely different from the rest of the use...
...
Accordingly, follow good practices for alt-text:
  ...
  * Images should usually have alt-text under 100 ch.note 21 Save longer descriptions for a caption or `aria-describedby`. Exceptions exist; this is just a weak norm. If longer descriptions elsewhere aren’t an option, disregard this advice and make alt-text as long as it needs to be.
...
A StackOverflow thread about comic transcripts outlines a good approach to semantically grouping images and transcripts, and my approach is similar. I group an image, alt-text, and caption in a `<figure>` element and follow it with a transcript in a `<details>` element. I use `aria-describedby` to semantically link the figure and the transcript.note 23
...
A second opinion: Chris Siebenmann describes this in more detail in A Web Colours Problem. In short: when setting colors, always set both the foreground and the background color. Don’t set just one of the two.
...
—Martin Mengele, Accessibility Issues concerning Windows High Contrast Mode
### Dark themes
...
Yellow may have great contrast on dark backgrounds, but vivid yellow and red can cause problems among people who deal with overstimulation; this includes many on the autism spectrum. Lē Silveus McNamara wrote about the issue on the TPGi blog: Beyond WCAG: Losing Spoons Online.note 26
...
Color palettes need to be effective for different types of vision deficiencies (e.g. color blindnesses) and screens. Color blindness is a far more nuanced topic than “the inability to see some colors”. Rob Pike describes his experience in Color blindness. Color blindness manifests in complex ways. Testing in grayscale is a great start, but it doesn’t account for all kinds of color vision deficiencies.
...
Moreover, several parts of  recommend underlining links.
...
—Clearly Identify Controls and Their Use: How it Helps
...
—Clearly Identify Controls and Their Use: What to Do
One key difference between buttons and links is the cursor appearance. Buttons should not turn the cursor into a “hand” or “pointer” icon. The article Some pointers on default cursors by Hidde de Vries covers both sides of the issue. Personally, I think it’s best to respect the convention of the browser and OS rather than break it.
Read more about the differences between buttons and links in Buttons vs. Links by Eggbert.
...
Normally, if you select some text in a page and press `Tab`, the tab-focusable element _after_ the selected text will receive focus. However, if the selectable text is inside a focusable container—even a container with a negative `tabindex`—pressing `Tab` will move focus to the _start_ of the container. If you’re reading this on a desktop browser, you can experience this first-hand: select some text in this paragraph and press `Tab`. Then, do the same in this snapshot of an excellent _Smashing Magazine_ article.note 30
Two screenshots of the aforementioned _Smashing Magazine_ article after I selected the title text, before and after pressing `Tab`. The focus moves _backwards_ to the start of the container because the container is focusable. The focus should have moved to an element after the selected text.
...
Two tools that can optimize the size of an SVG file are SVGO and the now-discontinued svgcleaner. Too much lossy SVG compression can sometimes _reduce_ the effectiveness of gzip and Brotli compression. Compress in moderation.
## Layout
Permalink to section
...
Websites following this page’s layout advice shouldn’t need much adjustment. Ahmed Shadeed’sRTL Styling 101 is a comprehensive guide to what can go wrong and how to fix issues.
### Current limitations
...
**Code snippet 10** (CSS): I do not re-style `:focus` when `:focus-visible` works, to match existing behavior. I also override `:focus` styling only on the subset of focusable elements that would normally show an outline. Based on the post :focus-visible and backwards compatibility by TPGi. 
```
a:focus,
[tabindex]:focus {
  outline: 3px solid;
...
—Charles McCathieNevile, longdesc Re: Clarification of rational for deprecation…
...
**Code snippet 11** (HTML): An example of using `role="text"` to prevent splitting in a multi-line heading. By Hampus Sethfords from Axess Lab, Text Splitting Causes Screen Reader Problems 
```
<h1>
...
—Steve Faulkner from TPGi, Short note on what CSS display properties do to table semantics
## Future users
Permalink to section
...
Webhint 
    Similar to Lighthouse. Again, you can ignore the accessibility audits if you already use axe-core. I personally disagree with some of its hints: the “unneeded HTTP headers” hint ignores the fact that the CSP can have an effect on non-hypertext assets, the “HTTP cache” hint has an unreasonable bias against caching HTML, and the “Correct `Content-Type` header” recommends charset attributes a bit too agg­ressively.note 39 
...
These tests begin reasonably, but gradually grow absurd. Once again, use your judgement.
  ...
  2. Evaluate the heaviness and complexity of your scripts (if any) by testing with your browser’s JIT compilation disabled.note 40
  ...
  16. Download your webpage and test how multiple word processors render and generate PDFs from it.note 41
...
This article is, and will probably always be, an ongoing work-in-progress. Some areas I have yet to cover:
  ...
  * Spatial navigation
  * The principle of redundant coding
  * Other accessible writing tips, maybe after I get a copy of Writing Is Designing by Michael Metts and AndyWelfe. A relevant excerpt on writing accessibly is on A List Apart.
...
—David Copeland, Brutalist Web Design
...
The 10KB Club did the same with a 10kb homepage budget (excluding favicons and webmanifest icons). It also had guidelines for note­worthiness, to avoid low-hanging fruit like mostly-blank pages.
...
One resource I found useful (that eventually featured this article!) was the “Your page content” section of Your Personal Website by Bill Dietrich.
If you’ve got some time on your hands, I _highly_ recommend reading the Web Content Accessibility Guidelines (WCAG) 2.2. The WCAG 2 standard is technology-neutral, so it doesn’t contain Web-specific advice. For that, check the How to Meet WCAG (Quick Reference). It combines the WCAG with its supplementary list of techniques.
The WCAG are an excellent starting point for learning about accessibility, but make for a poor stopping point. Much of the content on this page simply isn’t covered by the WCAG. One of my favorite resources for learning about what the WCAG _doesn’t_ cover is Axess Lab’s articles.
I’ve learned about a great number of under­represented ways to browse from the Fediverse, particularly from this subthread asking people to share (requires JavaScript; plaintext mirror). Several responses informed updates to this page.
...
High-Performance Browser Networking by Ilya Grigorik gives a great introduction to how TCP works, if you’d like more details. 
Back to reference 6
...
Back to reference 14
  15. Can I Use is operated by a single person; check their site footer for donation information. 
...
Back to reference 16
  17. Firefox users can enable “find as you type” by toggling a preference in about:config. Chromium (and derivatives) users can install an extension like Type-ahead-find; note that it requires full-page access and performs script injection to work. 
Back to reference 17
  18. Iterating through a list of font names to see if each one is available on a user’s system is a slow but effective way to determine installed fonts without being granted permission to use the Font Access API. Browser­Leaks has a demo of this approach. Warning: the page might hog your CPU for a while. 
Back to reference 18
  19. Decoration is more than cosmetic. The color overrides and accessibility sub-section describes how some decorations, like borders, improve access­ibility. 
...
Back to reference 20
  21. WebAIM and the University of Illinois recommend 100 characters; Tangaru recommends an even smaller limit of 80 characters. I sometimes exceed 100 characters for detailed images but usually stay below 80. 
...
Back to reference 22
  23. Once it gains basic support across all browsers and screen readers, I might recommend using `aria-details` instead of `aria-describedby` for more complex descriptions. At the time of writing, `aria-details` is only supported by JAWS. WAI-ARIA 1.2 describes `aria-details` with an example similar to the one I gave in code snippet 5. 
Back to reference 23
  24. Since WHCM sets colors independently of explicitly-defined ARIA roles, it’s a good way to test adherence to the First Rule of ARIA. 
...
Back to reference 25
  26. Lē also shared their experience in A11y Rules, one of my favorite podcasts 
...
Back to reference 27
  28. only renders invisible text without JavaScript. You can use a textual browser, screen reader, copy-paste the page contents elsewhere, use a reader-mode implementation, or “view source” to read it without enabling scripts. All of these options will ironically override the carefully-crafted typography of this website about typography.
...
Back to reference 34
  35. At least, it will be until NVDA bug 9343 gets resolved. 
Back to reference 35
  36. If you’d like to learn more, A guide to designing accessible, WCAG-compliant focus indicators by Sara Soueidan has far more details on making accessible focus indicators. 
...
Back to reference 37
  38. I excluded PageSpeed Insights and GTMetrix since those are mostly covered by Lighthouse. I excluded Hardenize and CryptCheck, since their scope is covered by Internet.nl.
I excluded Security Headers, since its approach seems to be recommending headers regardless of whether or not they are necessary. It penalizes forgoing the `Permissions-Policy` header even if the CSP blocks script loading and execution; see Security Headers issue #103. I personally find the `Permissions-Policy` header quite problematic, as I noted in August 2021 on webappsec-permissions-policy issue #189. Finally, Security Headers doesn’t have in-depth checks of the _values_ of headers; Internet.nl does a much better job of that. Security should be a thoughtful process, not a checklist. 
Back to reference 38
...
Send a WebmentionPublish a response on your own website, and link back to this page's canonical location. Then share your link here to turn it into a Webmention. URL of page linking here
Toggle 70 Webmentions
Webmentions received for this post appear in the following list after I approve them. I sometimes send Webmentions to myself on behalf of linking sites that don’t support them. I auto-replace broken links with Wayback Machine snapshots, if they exist. 2021-12-14 
     "It's been a while since I last looked at @Seirdy@…" - FLOSS.social by Adrian Cochrane
"It's been a while since I last looked at @Seirdy 's opinionated list of website best practices. It's expanded a bit, in what I view as a positive way: https://seirdy.one/2020/11/23/website-best-practices.html" 2022-01-31 
     Web design | Suzy McHale 
     Hyperlinks! — Computer-aided Art by Hans Verhaegen 2022-02-06 
     Chris X Edwards - xed.ch 2022-02-11 
     2021 Site Update by Roy Tang
"Over the past couple of weeks or so, I've made quite a few changes to the site, mostly focused on frontend/layout/UI changes. The changes are more or less done, so here's a changelog entry. The main driver of the changes was this post about best practices for text websites. Not all of it applies to the site, since I do have a bunch of image content around as well, but enough that I was spurred to apply many of the points and also include some additional changes I've been meaning to do. Second…" 2022-02-14 
     I realize not everybody’s going to ditch the Web and switch to Gemini or Gopher today (that’ll take, like, a month at the longest). by momonga 
     Short question regarding lazy loading: Native lazy loading loads images based on many factors and if you're on a (more or less) … by Snapstromegon
"Short question regarding lazy loading: Native lazy loading loads images based on many factors and if you're on a (more or less) fast connection, it will load everything when the network becomes idle (depending on device, Browser and more). I think it's better to not force everyone who might not need all images to load all (maybe, because he/she only reads the first two paragraphs and then clicks away). Why do you think everyone needs to load every image?" 2022-02-26 
     Links — 0xFF 2022-03-21 
     — Jacky Alciné 2022-03-27 
     @Seirdy Checking your site in Opera Mini (run it in microemulator) is a good idea, it's pretty common on feature phones, and … by easrng@cybre.space
...
"Checking your site in Opera Mini (run it in microemulator) is a good idea, it's pretty common on feature phones, and while it's a privacy shitshow it does do what it's supposed to (load pages without using much data)." 2022-03-29 
     An opinionated list of best practices for textual websites · Mirage 2022-04-18 
     @Seirdy @montagsoup Thanks for the explanation!I just refactored my site, and now I am going to again :D by Mark Esler
This comment may have major formatting errors that could impact screen reader comprehension.
"@montagsoup Thanks for the explanation!I just refactored my site, and now I am going to again :D" 2022-04-19 
     Website Design | Mark Esler 2022-04-21 
     Dear Apple, Please Fix Safari's Default Dark Mode Link Color · tdarb.org 2022-04-22 
     Site will be Redone in Hugo · Issue #750 · LukeSmithxyz/based.cooking · GitHub 2022-04-29 
     @Seirdy @IceWolf @LunaDragofelis @root @topaz my point is not that it is too complicated and should be made simpler somehow. my … by binarycat, silicon artificer
This comment may have major formatting errors that could impact screen reader comprehension.
"@IceWolf @LunaDragofelis @root @topaz my point is not that it is too complicated and should be made simpler somehow. my point is that creating good semantic markup requires considering a bunch of different things, and i think that’s part of why it isn’t super popular." 2022-05-04 
     Chris's Wiki :: blog/links/TextWebsiteBestPractices 2022-05-05 
     home/design.md at gh-pages · jon-jacky/home · GitHub 2022-05-09 
     i tested #fediverseInfo's home page using @Seirdy's battery of tests for websites … by salarua@koyu.space
This comment may have major formatting errors that could impact screen reader comprehension.
"i tested #fediverseInfo's home page using @Seirdy's battery of tests for websites (https://seirdy.one/2020/11/23/website-best-practices.html#unorthodox-tests). it failed on test 3. you'd think that being a guide to the Fediverse, its designers would know basic website design fediverseinfo Best practices for inclusive textual websites" 
     @SeirdyThanks for the insight and the text on your website. I try to be better on my websites (if the functionality permits it). @alexandra @Hiker by Birgit 🍀
This comment may have major formatting errors that could impact screen reader comprehension.
"Thanks for the insight and the text on your website. I try to be better on my websites (if the functionality permits it). @alexandra @Hiker" 2022-05-20 
     Opinions | Forever's Home 2022-05-28 
     Tiny Brain Fans - Accessibility 2022-06-08 
     Resize and reflow by Rohan Kumar
...
"Useful tools for blogs and websites. Part 6. Accessibility is very important to me, and I like to make sure that each web page I build is optimised for readability and supports screen readers. Fun fact: this blog is currently using the write.as standard style sheet and is not as accessible as my website on tilde.club. There is a lot to read and research about accessibility, inclusive websites, and accommodation and I definitely suggest the following websites as a starting point on accessi…" 2022-07-20 
     X_Cli: "@bortzmeyer@mastodon.gougere.fr @marien@tutut.del…" - Infosec Exchange by X_Cli
"@bortzmeyer @marien 🤷 Je ne suis pas convaincu qu'il faille céder à ce genre de pressions. Il est toujours possible de répondre "Certes, mais mon site web est accessible, et l'accessibilité couvre notamment la capacité à le surfer depuis des pays qui n'ont pas de bonnes connexions ou des mauvais forfaits".Un petit lien d'intérêt : https://seirdy.one/posts/2020/11/23/website-best-practices/" 2022-08-08 
     Best practices for inclusive textual websites | Lobsters by Seirdy 2022-08-10 
     frontendfoc.us/issues/554 2022-08-15 
     Issue #305
"Featured: Accessibility and QR codes“This blog post explores considerations and provides guidance for creating accessible experiences with QR codes.”Read more of Accessibility and QR codes.News, resources, tools and tutorialsI was asked to speak about diversity on a design panel that lacked diversity (blog post – uxdesign.cc)How our organization improved web accessibility (case study) (blog post – smashingmagazine.com)Watch my live accessibility review of KEXP.org (blog post - test…" 2022-08-16 
     Adrian Cochrane: "Reading over Seirdy's "Best practices for inclusi…" - FLOSS.social by 
"Reading over Seirdy's "Best practices for inclusive textual websites", some webstandards he mentions that I am supporting or plan to support in my browserengines relating to performance, accessibility, & privacy:* data: URIs* TLS 1.3* caching headers* I can be talked into CSS Containment, a prerequisite for Container Queries* Darkmode (and alternate styles)* Both web & native fonts* I don't do preloading, lazy-loading, or CSP.* Basically all HTML5 elements1/2" 2022-09-04 
     Swift icon 2022-09-12 
     harvey's | 14kB 2022-09-26 
     Remember, people override colors and fonts (even system fonts). Your colors and fonts are nothing more than a suggestion.Im … by 
This comment may have major formatting errors that could impact screen reader comprehension.
"Remember, people override colors and fonts (even system fonts). Your colors and fonts are nothing more than a suggestion.Im fact, your whole stylesheet is just a suggestion, but compatibility with CSS-disabling in a graphical browser is a separate topic." 
     @Seirdy btw there used to be an argument that hyperlinks slowed reading speed … by Christina Sørensen
This comment may have major formatting errors that could impact screen reader comprehension.
...
"[…] here’s a non-exhaustive, highly-opinionated list of best practices for websites that focus primarily on text. I don’t expect anybody to fully agree with the list; nonetheless, the article should have at least some useful information for any web content author or front-end web developer.~ Rohan Kumar from, https://seirdy.one/2020/11/23/website-best-practices.html Zoinks! Just reading the few paragraphs in Kumar’s Introduction suddenly renewed my pride at being among the few huma…" 2022-10-22 
     2022-11-09 
     @Seirdy @tricia I dig this article by Tobskep 2022-11-10 
     "Thank you so much 🙏🏻 thi…" - Front-End Social by Tricia (she/her)
...
"(The font thing *is* a neurodivergence thing for me, though not any of the specific ones you listed there. I just react extremely poorly to computers trying to do Special Branded Experiences. I don't know if there's a specific name for this besides "grumpy old lady yelling at technology".)" 2022-11-26 
     Best practices for inclusive textual websites - Seirdy https://seirdy.one/posts/2020/11/23/website-best-practices/ Best … by 𒀭𒂗𒆠
This comment may have major formatting errors that could impact screen reader comprehension.
"Best practices for inclusive textual websites - Seirdy https://seirdy.one/posts/2020/11/23/website-best-practices/ Best practices for inclusive textual websites" 2022-12-20 
     @matuzo @Edent If crossposts are okay, feel free to re-use this … by 
This comment may have major formatting errors that could impact screen reader comprehension.
"@matuzo @Edent If crossposts are okay, feel free to re-use this one:https://seirdy.one/posts/2020/11/23/website-best-practices/It’s long, but you can pick a section. I recommend the “Beyond Alt Text” section which describes a way to semantically combine brief alt text, comprehensive transcripts, and context-giving figure captions for more complex images.It’s all CC-BY-SA licensed. I could edit the section into a standalone post, if you’d like. Best practices for inclusive textual we…" 
     @alilly Dark-mode links (and with some GTK themes, focus outlines) have poor contrast on WebKit-based browsers. Wrote about it … by 
This comment may have major formatting errors that could impact screen reader comprehension.
"@alilly Dark-mode links (and with some GTK themes, focus outlines) have poor contrast on WebKit-based browsers. Wrote about it in the “dark themes” section of https://seirdy.one/posts/2020/11/23/website-best-practices/#dark-themesIf you offer a dark mode, I’d recommend against using default colors. Best practices for inclusive textual websites" 
     @Seirdy oh for FUCK'S SAKE guys *head into desk* by Athena Lilith 
     @alilly yup the WebKit Bugzilla and the CSSWG have had an open issue for this for years. Chromium and Firefox updated their dark … by 
This comment may have major formatting errors that could impact screen reader comprehension.
"@alilly yup the WebKit Bugzilla and the CSSWG have had an open issue for this for years. Chromium and Firefox updated their dark stylesheets; WebKit has left it on read i mean, not gotten back." 2022-12-24 
     A11y 2023-05-26 
     Bread Crumbs ·
"An excellent set of bests practices for “textual websites”: seirdy.one/2020/11/2… (yeah, after trying all the static website generators, it’s static-html-or-bust for me too)" 
     Node [[a11y]] in anagora.org 
     Writing - HTML tags versus CSS classes: is one preferred ... 2023-07-17 
     Clear Search 2023-07-22 
     Web Performance Calendar » Document Policy and Performance 2023-07-23 
...
"I spent last weekend poking around the outer edges of devices, browsers and websites, trying to find a way to make this site display nicely in as many of those places as possible. Of course, we can’t test every browser and device, but it’s worth trying some outside your everyday laptop or phone. In this case I opted for the Nokia 8110 and my TV’s browser. Turns out even very simple sites can struggle to work everywhere. While designing for all screen dimensions is doable, accounting fo…" 2023-07-24 
     @Seirdy this is very reassuring and calming after having read your "Best practices for inclusive textual … by max frühschütz – нет войне
This comment may have major formatting errors that could impact screen reader comprehension.
"this is very reassuring and calming after having read your "Best practices for inclusive textual websites"https://seirdy.one/posts/2020/11/23/website-best-practices/#use-the-advanced-perceptual-contrast-algorithmi am putting off releasing my own page since at least 3 weeks now because i always find a next issue to look at.i crave perfection but alas i will probably never reach it. time to release soon and document newly found issues in my issue tracker :) Best practices for inclusive…" 2023-08-03 
     @chemelia hmm…I wrote a big fat thing; you can skip around with the table of contents: … by 
This comment may have major formatting errors that could impact screen reader comprehension.
"@chemelia hmm…I wrote a big fat thing; you can skip around with the table of contents: https://seirdy.one/posts/2020/11/23/website-best-practices/It’s not prescriptive advice; it’s intended to establish a baseline that you can then diverge from and account for tradeoffs.The APCA Readability Criterion lists a ton of considerations. I’m not fully convinced on all of them but it’s a great reference when picking a color scheme and typography. Best practices for inclusive textual websites" 2023-08-18 
     @Seirdy i'll disable it for now. checking your website, it looks like people with hand tremors like to have an area they can … by Fries
This comment may have major formatting errors that could impact screen reader comprehension.
"i'll disable it for now. checking your website, it looks like people with hand tremors like to have an area they can safely touch, and that would take that away. i might just remove the click on post feature entirely." 2023-08-20 
     Website Best Practices by Brenton Cleeland
"This post discusses practices promoted by checks in ready, a tool that I've made to help developers check for security best practices in their websites. If you're working on a team that's building something served over HTTP you should check it out! Table of Contents This table of contents also serves a quick guide / checklist. Redirect HTTP traffic to HTTPS Cookies should be set securely HSTS headers must be returned You should support IPv6 Return a decent Content-Security-Policy Permissi…" 2023-08-24 
     @researchfairy Seriously, bespoke widgets have a massive tradeoff. Default widgets are easy to customize. Ask yourself if your … by 
This comment may have major formatting errors that could impact screen reader comprehension.
"@researchfairy Seriously, bespoke widgets have a massive tradeoff. Default widgets are easy to customize. Ask yourself if your app really needs that custom widget. Let the user pick their palette." 2023-09-03 
     Educational Sensational Inspirational Foundational 2023-09-06 
     Best practices for inclusive textual websites - @Seirdy: https://seirdy.one/posts/2020/11/23/website-best-practices/Long, but I … by 
This comment may have major formatting errors that could impact screen reader comprehension.
"Best practices for inclusive textual websites - @Seirdy: https://seirdy.one/posts/2020/11/23/website-best-practices/Long, but I think I'm largely in agreement... Best practices for inclusive textual websites" 2023-11-09 
     kangae: a lightweight microblog theme for zola 2023-11-18 
     CSS Breakdown 2023-11-22 
     @Seirdy Two issues I have with underlining links globally:I prefer a value of 0.25em for the text-underline-offset rather than … by Ayush Agarwal
This comment may have major formatting errors that could impact screen reader comprehension.
"Two issues I have with underlining links globally:I prefer a value of 0.25em for the text-underline-offset rather than the default which makes me uncomfortable for some reason. So if I'm going to globally underline links, I'd like to set this CSS property as well, although it may not work well on all websites.on pages with dozens of (underlined) links, reading can become an uncomfortable experience as showcased by marginlia on their blog. Thoughts on the linkpocalypse" 
     @Seirdy@pleroma.envs.net from an article linked in the your articleOf course the article shitting on underlines has … by ikoli
This comment may have major formatting errors that could impact screen reader comprehension.
"from an article linked in the your articleOf course the article shitting on underlines has unrecognizable links. Doesn't even use a intuitive icon for the link. I unironically stared at them for a few seconds before I tried hovering them lol" 2024-05-21 
     @Seirdy on Android Firefox what I think are supposed to be on-hover pop up text tips don't work: e.g. "HTML (POSH)" in … by matt
This comment may have major formatting errors that could impact screen reader comprehension.
"on Android Firefox what I think are supposed to be on-hover pop up text tips don't work: e.g. "HTML (POSH)" in https://seirdy.one/posts/2020/11/23/website-best-practices/I couldn't find a way to interact with POSH and have it show me the definition. Open question if this is a FF bug or webmaster problem. Either way, you probably want to know. Best practices for inclusive textual websites" 
     @maphew It’s an issue in mobile browsers in general, and probably a WONTFIX on their end. I define initialisms/acronyms the … by 
This comment may have major formatting errors that could impact screen reader comprehension.
"@maphew It’s an issue in mobile browsers in general, and probably a WONTFIX on their end. I define initialisms/acronyms the first time I use them as fallbacks but if I skipped one please let me know." 
     @easrng @maphew This uses CSS features and syntax a decade ahead of what I’m currently using /hj.I treat the browser-native … by 
This comment may have major formatting errors that could impact screen reader comprehension.
"@easrng @maphew This uses CSS features and syntax a decade ahead of what I’m currently using /hj.I treat the browser-native hover effects (and CSS itself) as an optional enhancement but I avoid requiring them to understand an article." 2024-07-14 
     shoutout to this accessibility for textual websites guide for alerting me to the various issues + linking to the observatory, … by Him Tortons
This comment may have major formatting errors that could impact screen reader comprehension.
"shoutout to this accessibility for textual websites guide for alerting me to the various issues + linking to the observatory, btw https://seirdy.one/posts/2020/11/23/website-best-practices/ Best practices for inclusive textual websites" 
     Why I won't upgrade to Cockpit CMS v2 and might leave the project - #4 by raffaelj - General - Cockpit Community 2024-08-10 
     @Seirdy Still reading your post: https://seirdy.one/posts/2020/11/23/website-best-practices/I'm thinking if splitting it into … by Dillo browser
This comment may have major formatting errors that could impact screen reader comprehension.
"Still reading your post: https://seirdy.one/posts/2020/11/23/website-best-practices/I'm thinking if splitting it into separate parts (~30 min) may help the readers.> 23 thousand words; a very long 1 hour and 51 minute readSide question: do you have a view on how to write math on the web? Specifically, without JavaScript support.From Dillo I don't think we could implement anything close to MathML any time soon, but we have some patches to support SVG equations like the ones in Wikipedi…" 
     @dillo yeah it’s more of a short book than a blog post. use the ToC to split it up or read out of order. by 
This comment may have major formatting errors that could impact screen reader comprehension.
...
* * *
## Continue reading
  * Previous post: Resilient Git, Part 1: Hydra Hosting
  * Next post: Becoming physically immune to brute-force attacks
...
You are here: 
  1. Articles
  2. Best practices for inclusive textual websites
