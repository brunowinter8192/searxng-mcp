...
# ------------------------------------------------------------------
    ...
    respraise_for_status() # It ensures we only proceed if the HTTP response was successful (200 OK).
    ...
    return arttextstrip() if arttext else None # Return the main text or None
...
# ------------------------------------------------------------------
    ...
    # If all methods fail, return an empty string
            if text and lentext  MIN_CONTENT_LENGTH # Check for sufficient content
        ...
        display_text_color"Could not extract meaningful content" ForeRED
