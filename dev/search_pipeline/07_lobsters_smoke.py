#!/usr/bin/env python3
"""Lobsters smoke test — self-contained, reads config.yml, no src/ imports."""
...
# Start browser with fingerprint patches applied (no consent cookie needed for Lobsters)
...
async def run_query(browser: Chrome, query: str, cfg: dict) -> dict:
    ...
    try:
        search_url = lc["url_template"].format(query=quote_plus(query))
        ...
        found = await _wait_for_results(tab, lc)
        ...
        # Parse results
        results = await _parse_results(tab, lc)
...
async def _wait_for_results(tab, lc: dict) -> bool:
    js = lc["selectors"]["wait_js"]
    max_cycles = lc["wait_for_results"]["max_cycles"]
    interval = lc["wait_for_results"]["interval_seconds"]
...
async def _parse_results(tab, lc: dict) -> list[dict]:
    js = lc["selectors"]["parse_js"]
    raw = await tab.execute_script(js)
...
def _derive_status(count: int, domains: int, title: str) -> str:
    if count == 0 and "Lobsters" not in title:
        return "BLOCKED"
    if count == 0:
        return "EMPTY"
