#!/usr/bin/env python3
"""Mojeek smoke test — self-contained, reads config.yml, no src/ imports."""
...
# Start browser with fingerprint patches applied (no consent cookie needed for Mojeek)
...
# Run one query: navigate, wait for results, parse, return record dict
async def run_query(browser: Chrome, query: str, cfg: dict) -> dict:
    ...
    try:
        search_url = mc["url_template"].format(query=quote_plus(query))
        ...
        found = await _wait_for_results(tab, mc)
        ...
        # Parse results
        results = await _parse_results(tab, mc)
...
async def _wait_for_results(tab, mc: dict) -> bool:
    js = mc["selectors"]["wait_js"]
    max_cycles = mc["wait_for_results"]["max_cycles"]
    interval = mc["wait_for_results"]["interval_seconds"]
...
async def _parse_results(tab, mc: dict) -> list[dict]:
    js = mc["selectors"]["parse_js"]
    raw = await tab.execute_script(js)
...
def _derive_status(count: int, domains: int, title: str) -> str:
    if count == 0 and "Mojeek" not in title:
        return "BLOCKED"
    if count == 0:
        return "EMPTY"
...
def write_report(records: list[dict], report_dir: Path) -> Path:
    ...
    path = report_dir / f"mojeek_smoke_{ts}.md"

    lines = [
        f"# Mojeek Smoke Test — {ts}",
        "",
