...
| `11_pipeline_smoke.py` | Full-pipeline smoke — calls `search_web_workflow` per query (vs 05's per-engine fanout that bypasses `_merge_and_rank`), reads `cache_read` after each call to extract structured per-URL data with `engines: list[str]` aggregation. Outputs slot-position labels (`[GENERAL]`/`[ACADEMIC]`/`[QA]`/`[OVERFLOW?]`), engines list, snippet, plus per-query slot-fill summary line `Slot fill: GENERAL X/12, ACADEMIC Y/4, QA Z/2, OVERFLOW W/2`. Writes `pipeline_smoke_<ts>.md` to `01_reports/`. CLI flags: `--max-queries N`, `--language` (default `en`). Use this script (not 05) when reviewing the production ranking output as the user would see it. |
...
| `snippet_quality_analysis.py` | Per-source bloat + lexical-density analysis over a multi-engine smoke report. 10 sources (8 engines + og + meta preview), 9 bloat-pattern regexes (URL breadcrumb, Read-more, Web-results prefix, Featured-snippet prefix, social-proof, Scholar ellipsis, Mojeek nav-dump, HTML entities, Tag noise) + 1 JATS-XML pattern. Output: per-source aggregated stats table + 8×8 engine-overlap matrix + side-by-side examples for ≥3-source URLs. EN+DE combined stopword list for lexical_density (avoids penalizing German queries). Inputs the canonical baseline smoke (`search_smoke_*.md`). Output `snippet_quality_<ts>.md` to `01_reports/`. |
| `01_reports/` | Per-run markdown reports — `google_smoke_*.md`, `ddg_smoke_*.md`, `mojeek_smoke_*.md`, `lobsters_smoke_*.md`, `scholar_smoke_*.md`, `openalex_smoke_*.md`, `se_smoke_*.md`, `search_smoke_*.md`, `burst_*.md`, `empty_classify_se_*.md`, `empty_classify_lobsters_*.md`, `snippet_quality_*.md`, `search_web_demo_*.md` plus `empty_classify_lobsters_screenshots/<query>.png` |
