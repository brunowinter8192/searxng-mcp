#!/usr/bin/env python3
"""Semantic Scholar smoke test — invokes SemanticScholarEngine().search() for 30 baseline queries."""
...
async def run_query(engine: SemanticScholarEngine, query: str) -> dict:
...
# Classify result count into status label
...
def write_report(records: list[dict], report_dir: Path) -> Path:
    ...
    path = report_dir / f"semscholar_smoke_{ts}.md"

    ok_count = sum(1 for r in records if r["status"] == "OK")
    lines = [
        f"# Semantic Scholar Smoke Test — {ts}",
        "",
