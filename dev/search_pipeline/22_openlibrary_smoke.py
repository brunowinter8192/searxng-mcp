#!/usr/bin/env python3
"""Open Library smoke test — invokes OpenLibraryEngine().search() for 30 baseline queries."""
...
async def run_query(engine: OpenLibraryEngine, query: str) -> dict:
    record: dict = {"query": query, "count": 0, "sample_urls": [], "sample_titles": [], "status": "EMPTY", "elapsed_ms": 0}
    try:
        results = await engine.search(query)
        record["count"] = len(results)
        record["sample_urls"] = [r.url for r in results[:3]]
        record["sample_titles"] = [r.title for r in results[:3]]
...
def write_report(records: list[dict], report_dir: Path) -> Path:
    ...
    path = report_dir / f"openlibrary_smoke_{ts}.md"

    ok_count = sum(1 for r in records if r["status"] == "OK")
    lines = [
        f"# Open Library Smoke Test — {ts}",
        "",
