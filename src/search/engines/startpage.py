...
SEARCH_URL = "https://www.startpage.com/sp/search?query={query}&language={language}"
WAIT_SECONDS = 3.0
...
# Search Startpage via pydoll stealth browser and return ranked results
class StartpageEngine(BaseEngine):
    ...
    async def search(self, query: str, language: str = "en", max_results: int = 10) -> list[SearchResult]:
        ...
        items = await _fetch_results(query, language)
        if items is None:
            limiter.backoff()
            return []
...
async def _fetch_results(query: str, language: str) -> list[dict] | None:
    ...
    try:
        ...
        if not items:
            logger.warning("Startpage: no results extracted from %s", url)
        return items
